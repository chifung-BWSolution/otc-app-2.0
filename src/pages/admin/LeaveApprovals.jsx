...
export default function LeaveApprovals() {
  ...
  const loadAll = async () => {
    setLoading(true);
    const data = await base44.entities.LeaveRequest.list("-created_date", 100);
    setRecords(data);
    setLoading(false);
  };
  ...
  const pending = records.filter((r) => r.status === "待審批").length;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-green-50 rounded-xl p-3 text-center border border-green-100">
          <div className="text-2xl font-bold text-green-600">{records.filter((r) => r.status === "已批准").length}</div>
          <div className="text-xs text-gray-500 mt-0.5">已批准</div>
        </div>
        <div className="bg-red-50 rounded-xl p-3 text-center border border-red-100">
          <div className="text-2xl font-bold text-red-500">{records.filter((r) => r.status === "已拒絕").length}</div>
              ...
              {item.status === "待審批" && (
                <div className="mt-3 space-y-2">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => handleDecision(item, "已批准")}
                    ...
                    <button
                      onClick={() => handleDecision(item, "已拒絕")}

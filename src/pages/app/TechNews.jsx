  ...
  return (
    <div className="space-y-0 -mx-4 md:-mx-6 -mt-4 md:-mt-6">
      ...
      <div className="relative bg-black overflow-hidden" style={{ height: 340 }}>
        {heroNews.map((n, i) => (
          <div key={n.id} className={`absolute inset-0 transition-opacity duration-700 ${i === heroIdx ? "opacity-100" : "opacity-0"}`}>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-white/70">{n.tag}</span>
        ...
        <button onClick={() => goHero(-1)} className="absolute left-3 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/60 text-white rounded-full p-1.5 z-10 transition-colors">
          <ChevronLeft size={18} />
        </button>
        <button onClick={() => goHero(1)} className="absolute right-3 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/60 text-white rounded-full p-1.5 z-10 transition-colors">
          <ChevronRight size={18} />
        </button>
        <div className="absolute bottom-3 right-5 flex gap-1.5 z-10">
          {heroNews.map((_, i) => (
            <button key={i} onClick={() => setHeroIdx(i)} className={`h-1.5 rounded-full transition-all ${i === heroIdx ? "bg-white w-4" : "bg-white/50 w-1.5"}`} />
          ))}
        </div>
      </div>

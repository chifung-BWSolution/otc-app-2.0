  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-blue-50 px-4 py-3 border-b border-indigo-100">
        <div className="text-base font-bold text-gray-800 truncate">{project.project_name}</div>
        <div className="flex items-center gap-3 mt-1">
          <span className="text-sm bg-white/80 text-indigo-600 px-2 py-0.5 rounded-full font-semibold border border-indigo-100">{project.hours}h</span>
          <span className="text-sm text-gray-500">{project.tasks} 個任務</span>
          {score > 0 && (
            <span className={`text-sm px-2 py-0.5 rounded-full font-semibold border ${SCORE_COLORS[score]?.bg} ${SCORE_COLORS[score]?.border} ${SCORE_COLORS[score]?.text}`}>
            ⭐ 自評 {score} 分
            </span>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-4 pt-4 pb-3">
          <div className="flex items-center gap-2 mb-3">
            ...
            <label className="text-base font-semibold text-gray-700">📝 貢獻重點 + 成長重點</label>
            <span className="text-sm text-gray-400 ml-auto">{points.filter(p => p.text?.trim()).length} 項已填寫</span>
          </div>
          ...
          <button
            onClick={onAddPoint}
            className="mt-2.5 flex items-center gap-1.5 text-sm text-indigo-600 font-semibold hover:text-indigo-800 transition-colors px-1"
          >
        ...
        <div className="px-4 pt-3 pb-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <label className="text-base font-semibold text-gray-700">⭐ 自評分數</label>
          </div>

          {!points.some(p => p.text?.trim()) ? (
            <div className="text-sm text-gray-400 bg-gray-50 rounded-lg px-3 py-3 text-center border border-dashed border-gray-200">
              請先填寫貢獻重點後才可自評分數
            </div>
          ) : (
            <>
              ...
              {score > 0 && (() => {
                ...
                return (
                  <div className={`mt-2 text-sm ${colors.text} ${colors.bg} rounded-lg px-3 py-2 border ${colors.border}`}>
    ...
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="px-3 pt-2.5 pb-2">
        <div className="flex items-center gap-1.5 mb-2">
          ...
          <span className="text-sm text-gray-500">選擇類型：</span>
          {canRemove && (
          ...
          {contributionTypes.map(ct => {
            ...
            return (
              <button
                ...
                className={`px-2.5 py-1.5 rounded-lg text-sm font-semibold transition-all border ${
                  isActive
      ...
      <div className="px-3 pb-2.5">
        <textarea
          className="w-full border border-gray-100 rounded-lg px-3 py-2 text-base focus:outline-none focus:ring-2 focus:ring-indigo-200 bg-gray-50/50 resize-none"
          rows={2}

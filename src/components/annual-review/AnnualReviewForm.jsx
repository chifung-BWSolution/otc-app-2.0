...
function parsePoints(note) {
  ...
  return [typeof note === "string" ? { type: "", text: note } : note];
}

function serializePoints(points) {
  const cleaned = points.filter(p => p.text?.trim());
...
export default function AnnualReviewForm({ projectSummary, existingReview, saving, onSave }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      base44.entities.ScoreLevel.filter({ is_active: true }, "sort_order", 100),
    ]).then(([ct, sl]) => {
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="bg-blue-50 px-5 py-4 border-b border-blue-100">
          ...
          <p className="text-sm text-blue-600 mt-0.5">
            左邊選擇項目查看任務明細，右邊填寫銷售數字、選擇貢獻類型並填寫重點，最後自評分數。
          </p>
        ...
        <div className="p-5">
          ...
          <div className="flex flex-col lg:flex-row gap-4 min-h-[480px]">
            {/* Left: Project list */}
            <div className="lg:w-[40%] flex flex-col border border-gray-200 rounded-xl overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto">
                {visibleIndices.map((projIdx) => {
                  ...
                  const hasFilled = p.sales_amount > 0 || points.some(pt => pt.text?.trim());
                      ...
                      >
                        ...
                        <div className="text-right shrink-0">
                          ...
                          <div className="flex items-center gap-1 mt-1 justify-end">
                            {hasFilled && <span className="w-2.5 h-2.5 rounded-full bg-green-400" title="已填寫貢獻" />}
                            {hasScore && <span className="w-2.5 h-2.5 rounded-full bg-amber-400" title="已自評" />}
                          </div>
                        </div>
                      </button>
            ...
            {/* Right: Sales + contribution points + self score */}
            <div className="lg:w-[60%] flex flex-col border border-gray-200 rounded-xl overflow-hidden">
              {sel && allIndices.includes(selectedProject) ? (
                <>
                  ...
                  <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
                    ...
                    {/* Contribution points with type selection */}
                    <div>
                        ...
                        {selPoints.map((pt, pi) => (
                          <div key={pi} className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                            <div className="flex items-center gap-2 mb-2">
                              <span className="text-xs text-gray-400 w-5 shrink-0 text-right font-bold">{pi + 1}.</span>
                              <select
                                className="border border-gray-200 rounded-lg px-2 py-1.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-300 flex-1"
                                value={pt.type || ""}
                                onChange={e => updatePoint(selectedProject, pi, "type", e.target.value)}
                              >
                                <option value="">選擇貢獻類型...</option>
                                {contributionTypes.map(ct => (
                                  <option key={ct.id} value={ct.name}>{ct.icon} {ct.name}</option>
                                ))}
                              </select>
                              {selPoints.length > 1 && (
                                <button onClick={() => removePoint(selectedProject, pi)} className="p-1 text-gray-300 hover:text-red-500 transition-colors shrink-0">
                                  <X size={14} />
                                </button>
                              )}
                            </div>
                            <input
                              ...
                              placeholder={pt.type ? `描述你在「${pt.type}」方面的具體貢獻...` : "請先選擇類型..."}
                              value={pt.text || ""}
                              onChange={e => updatePoint(selectedProject, pi, "text", e.target.value)}
                            />
                          </div>
                        ))}
                      </div>
                    ...
                    {/* Self Score */}
                    <div>
                      <label className="text-sm font-medium text-gray-700 block mb-2">⭐ 自評分數</label>
                      <div className="space-y-1.5">
                        {scoreLevels.map(sl => {
                          const isSelected = selScore === sl.score;
                          return (
                            <button
                              key={sl.id}
                              onClick={() => updateScore(selectedProject, sl.score)}
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="text-xs text-gray-400">{sl.description}</div>
                              </div>
                            </button>
                          );
                        })}

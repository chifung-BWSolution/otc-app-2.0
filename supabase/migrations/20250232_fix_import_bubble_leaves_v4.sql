-- ============================================================
-- Recreate import_bubble_leaves function WITHOUT deleted columns:
-- google_event_id, interview_email, send_email, send_approval_email, created_by_id, base44_id
-- Also uses 已批核/不批核 status values instead of English
-- ============================================================
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Lookup staff_id from staff table
      v_staff_id := NULL;
      ...
      -- Lookup approver_id from staff table
      v_approver_id := NULL;
      ...
      -- Lookup rejecter_id from staff table
      v_rejecter_id := NULL;
      ...
      -- Derive status (Chinese values)
      v_status := CASE
        WHEN v_approved = true THEN '已批核'
        WHEN v_approved = false THEN '不批核'
        WHEN v_rejecter_name IS NOT NULL THEN '不批核'
        ELSE '審查中'
      END;

      -- Map leave_period Bubble ID to code
      v_leave_period_code := CASE v_leave_period_bubble_id
        ...
        WHEN '1735808293079x986033414650986500' THEN 'CL'
        WHEN '1737106791087x793592504562483200' THEN 'NA'
        ELSE 'NA'
      END;

      -- Map leave_type Bubble ID to Chinese name
      ...
      VALUES (
        ...
        NULLIF(TRIM(COALESCE(rec->>'Reason for apply', rec->>'Application Reason')), ''),
        NULLIF(TRIM(rec->>'Reject Reason'), ''),
        NULLIF(TRIM(rec->>'Remarks'), ''),
        NULLIF(TRIM(rec->>'Prove'), ''),
        NULLIF(TRIM(COALESCE(rec->>'Info-Tech', rec->>'Info Tech')), ''),
        NOW(),
        NOW()
      )
      ON CONFLICT (bubble_id) DO UPDATE SET
        ...
        info_tech_url = EXCLUDED.info_tech_url,
        updated_date = NOW();

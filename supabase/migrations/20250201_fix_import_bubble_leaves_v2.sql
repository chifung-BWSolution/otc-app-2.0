-- ============================================================
-- Recreate import_bubble_leaves function with better error handling
-- and more defensive value extraction.
-- 
-- Key fixes:
-- 1. Handle "-Quota" key with different dash characters
-- 2. Better NULL handling for all fields
    ...
    BEGIN
      ...
      IF v_bubble_id IS NULL THEN
        ...
        IF jsonb_array_length(v_error_samples) < 5 THEN
          v_error_samples := v_error_samples || jsonb_build_object('error', 'missing bubble_id', 'rec_keys', (SELECT jsonb_agg(key) FROM jsonb_object_keys(rec) AS key));
        END IF;
        CONTINUE;
      END IF;
      ...
      -- Convert quota - try multiple possible key names for the -Quota field
      v_quota := NULL;
      v_quota_raw := NULL;
      
      -- Try different key variations for the quota field
      v_quota_raw := rec->>'-Quota';
      IF v_quota_raw IS NULL OR v_quota_raw = '' THEN
        v_quota_raw := rec->>E'\u2212Quota';  -- Unicode minus sign
      END IF;
      IF v_quota_raw IS NULL OR v_quota_raw = '' THEN
        v_quota_raw := rec->>E'\u2013Quota';  -- En dash
      END IF;
      IF v_quota_raw IS NULL OR v_quota_raw = '' THEN
        -- Try to find any key containing "Quota"
        SELECT rec->>k INTO v_quota_raw
        FROM jsonb_object_keys(rec) AS k
        WHERE k LIKE '%Quota%' OR k LIKE '%quota%'
        LIMIT 1;
      END IF;
